...
const toStyle = (props: BasicProps): CSSProperties => ({
  ...
  background:
    props.bg || props.bgColor
      ? (colorMap[(props.bg || props.bgColor) as string] ??
        (props.bg || props.bgColor))
      : undefined,
  border: props.border,
  ...
  flexDirection: (props.flexDir ??
    props.flexDirection ??
    props.direction) as CSSProperties['flexDirection'],
...
export const Heading = forwardRef<
  HTMLHeadingElement,
  BasicProps & { as?: string; size?: string }
...
export const Link = forwardRef<
  HTMLAnchorElement,
  BasicProps & { as?: any; href?: string; to?: string }
>(function Link({ as: Component = 'a', style, ...props }, ref) {
  return (
    <Component
      ref={ref}
      style={{ textDecoration: 'none', ...toStyle(props), ...style }}
      {...props}
    />
  )
})
...
>(function Button(
  { children, colorScheme, isDisabled, isLoading, leftIcon, variant, ...props },
  ref
) {
  const actualVariant =
    colorScheme === 'red'
...
export const Input = forwardRef<
  HTMLInputElement,
  InputHTMLAttributes<HTMLInputElement> & BasicProps
>(function Input({ style, ...props }, ref) {
  return (
    <UIInput ref={ref} style={{ ...toStyle(props), ...style }} {...props} />
  )
})
...
export function FormControl({
  children,
  isInvalid
}: { children: ReactNode; isInvalid?: boolean } & BasicProps) {
...
export function Alert({
  ...
  status = 'info',
  ...props
}: { children: ReactNode; status?: string } & BasicProps) {
  const className =
    status === 'error'
  ...
  return (
    <div
      className={`rounded-[var(--radius-md)] border px-3 py-2 ${className}`}
      style={toStyle(props)}
    >
      {children}
    </div>
...
export function Progress({
  ...
  max = 100,
  ...props
}: { value?: number; max?: number } & BasicProps) {
  const width = `${(value / max) * 100}%`
  return (
    <div
      className="h-2 w-full overflow-hidden rounded-full bg-[color:var(--color-border)]"
      style={toStyle(props)}
    >
      <div
...
export function Select({
  ...
  style,
  ...props
}: SelectHTMLAttributes<HTMLSelectElement> & BasicProps) {
  return (
    <select
...
export function Checkbox({
  ...
  return (
    <label
      ...
      style={toStyle(props as BasicProps)}
    >
...
export function ListIcon({
  ...
  return (
    <Component
      ...
      style={{ color: color ? (colorMap[color] ?? color) : undefined }}
    />
  )
}
...
export function MenuList({
  ...
  return (
    <div
      className="absolute z-50 mt-2 min-w-[180px] rounded-[var(--radius-md)] border bg-[color:var(--color-card)] p-1 shadow-lg"
      style={toStyle(props)}
    >
      {children}
    </div>
...
export function MenuItem({
  ...
  return (
    <button
      className="flex w-full items-center gap-2 rounded px-2 py-2 text-left hover:bg-[color:var(--color-accent)]/40"
      type="button"
      style={toStyle(props)}
      {...props}
    >
...
export function DrawerContent({
  ...
  return (
    <div className="h-full w-[320px] bg-[color:var(--color-card)]">
      {children}
    </div>
  )
}
...
export function AlertDialogContent({
  ...
  return (
    <div className="relative w-full max-w-md rounded-[var(--radius-lg)] border bg-[color:var(--color-card)] p-6 shadow-xl">
      {children}
    </div>
  )
}
export function AlertDialogHeader({
  ...
  return (
    <h2 className="text-lg font-bold" style={toStyle(props)}>
      {children}
    </h2>
  )
}
export function AlertDialogBody({
  ...
  return (
    <div className="mt-3" style={toStyle(props)}>
      {children}
    </div>
  )
}
export function AlertDialogFooter({
  ...
  return (
    <div className="mt-5 flex justify-end gap-3" style={toStyle(props)}>
      {children}
    </div>
  )
}
export function AlertDialogCloseButton({
  ...
  return (
    <button className="absolute right-4 top-4" onClick={onClick} type="button">
      x
    </button>
  )
}
