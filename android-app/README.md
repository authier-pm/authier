...
Android Autofill supports native app login forms with exact package associations, timed session recovery, password or fingerprint unlock, and account selection. Enable Authier from Settings. In another app, unlock Authier, choose or search for a saved login, and confirm “Use login and fill” to remember the app automatically. Linked accounts appear first on future requests; “Choose another saved login” links a different account. Each login currently supports one Android app; the confirmation explicitly identifies any link being replaced. Associations are encrypted and queued locally, then synced when Authier is next opened and unlocked. Items with pending writes must be synced or resolved before linking. Manual package entry remains available in the password editor. Autofill also supports HTTPS forms in the system-selected default browser and embedded WebViews whose website authorizes the installed app signing certificate through Digital Asset Links (`delegate_permission/common.get_login_creds`). Web logins match the exact scheme, host, and port of their saved website; a browser package is never saved as the association. Embedded-app verification needs connectivity. Unknown schemes, mixed-origin frames, and ambiguous password fields are excluded. Web origin reporting requires Android 9 or newer; native forms remain supported on Android 8.

For registration or password changes, tap **Create a strong password with Authier** in the autofill menu. Unlock if needed, review the username and generated password, regenerate or edit it, then choose **Save and fill**. A new encrypted login is saved locally and queued for sync before any value is returned to Android. Explicit new-password and confirmation fields receive the same value, while current-password fields remain unchanged. Ordinary single password fields also offer creation; fields explicitly marked current-password offer saved logins only. Canceling saves and fills nothing. This creates a separate login; existing vault passwords are never overwritten, and the target form still needs to be submitted by the user. The app offers only fields exposed through Android Autofill, not every password field visible on screen.
